...
def get_bytes_from_flag(flags: int, pattern: list[int] or int) -> int:
    if pattern is int:
        pattern = [pattern]
...
def NAV_SAT(msg: bytes, receiving_time: datetime or None = None) -> (MessageTypes, list):
    if receiving_time is None:
        receiving_time = datetime.now()
    header = '<LBBBB'
    iTOW, version, numsSvs, *reversed1 = struct.unpack(header, msg[:struct.calcsize(header)])
    data = Struct(type='NAV_SAT', sys_time=receiving_time, iTOW=iTOW, version=version, numsSvs=numsSvs, SatNAV=[])
    ...
    for i in range(numsSvs):
        ...
        SatData = Struct(gnssId=gnssId, svId=svId, cno=cno, elev=elev, azim=azim, prRes=prRes / 10.0,
                         qualityInd=bitfield(0, 1, 2),
                         svUsed=bitfield(3),
                         health=bitfield(4, 5),
                         ephAvail=bitfield(11),
                         almAvail=bitfield(12))
        data.SatNAV.append(SatData)

    return MessageTypes.NAV, data


def NAV_ORB(msg: bytes, receiving_time: datetime or None = None) -> (MessageTypes, list):
    if receiving_time is None:
        receiving_time = datetime.now()
    header = '<LBBBB'
    iTOW, version, numsSvs, *reversed1 = struct.unpack(header, msg[:struct.calcsize(header)])
    data = Struct(type='NAV_ORB', sys_time=receiving_time, iTOW=iTOW, version=version, numsSvs=numsSvs, SatORB=[])
    msg = msg[struct.calcsize(header):]
    ...
    for i in range(numsSvs):
        ...
        SatData = Struct(gnssId=gnssId, svId=svId, health=svFlag % 4, visibility=(svFlag // 4) % 4,
                         ephUsability=eph % 32, ephSource=eph // 32,
                         almUsability=alm % 32, almSource=alm // 32,
                         anoAopUsability=otherOrb % 32, type=otherOrb // 32
                         )
        data.SatORB.append(SatData)
...
def NAV_VELECEF(msg: bytes, receiving_time: datetime or None = None) -> dict[str: object]:
    ...
    data = Struct(type='NAV_POSECEF', sys_time=receiving_time, iTOW=iTOW, tAcc_ecefV=tAcc / 100,
