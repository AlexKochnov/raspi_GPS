...
sats_alm = readtable('data/alm_5_.csv');
% sats_eph = readtable('data/eph_5_.csv');
...
function lla = MYecef2lla(xyz)
    % Константы
    a = 6378137.0;          % Экваториальный радиус Земли в метрах
    e2 = 6.69437999014e-3;  % Квадрат эксцентриситета
    b = sqrt(a^2 * (1 - e2)); % Полярный радиус Земли
    ep2 = (a^2 - b^2) / b^2;  % Квадрат второго эксцентриситета
    ...
    % Вычисление широты и высоты
    lat = atan2(z + ep2 * b * sin(theta).^3, p - e2 * a * cos(theta).^3);
    N = a ./ sqrt(1 - e2 * sin(lat).^2);  % Радиус кривизны в вертикальной плоскости
    alt = p ./ cos(lat) - N;

    % Преобразование в градусы
    lla(:,1) = lat * (180 / pi);  % Широта
    lla(:,2) = lon * (180 / pi);  % Долгота
    lla(:,3) = alt;               % Высота в метрах
end
